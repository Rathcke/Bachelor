  ...
  \begin{algorithmic}[1]
    ...
    \Function{K-Dist}{$s, t, k$}
      ...
      \For{$i \gets 0$ to $length(s) - k$}
        ...
        \State $cur\_dist \gets $ \Call{\textsc{Update-Dist}}
...
\begin{algorithm}[H]
  ...
  \begin{algorithmic}[1]
    \Require{\texttt{m} is a map from \texttt{string} to \texttt{integer},
      $kmer\_add$ and $kmer\_sub$ are strings and $dist \in \mathbb{Z}$}
    \Statex
    \Function{Update-Dist}{$\mathtt{m},\,kmer\_add,\,kmer\_sub,\,dist$}
      \If{$\mathtt{arr}[kmer\_add] < 0$}
        \State $dist \gets dist - 1$
      \Else
        \State $dist \gets dist + 1$
      \EndIf
      \If{$\mathtt{arr}[kmer\_sub] > 0$}
        \State $dist \gets dist - 1$
      \Else
        \State $dist \gets dist + 1$
      \EndIf
