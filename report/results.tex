...
in the same cluster. The result was equally correct when running \texttt{klust}
with $id=0.9$.
...
The technique used for the dimensionality reduction is \emph{t-distributed
...
expected to be in the same cluster was given the same color and thus the MDS is
expected to give a visualization of separate clusters of points of the same
color.
...
\begin{figure}[H]
  ...
  \caption{Multidimensional scaling of the first 400 sequences (40 expected
    clusters) of the synthetic dataset.}
  \label{fig:mds_synth}
\end{figure}
